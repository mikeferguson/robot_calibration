...
#include <robot_calibration/ceres/chain3d_to_camera2d_error.h>
                        ...
                        bool progress_to_stdout)
{
  ...
  for (size_t i = 0; i < params.models.size(); ++i)
  {
    ...
    else if (params.models[i].type == "camera2d")
    {
      ...
      Camera2dModel* model = new Camera2dModel(params.models[i].name, param_name, tree_, params.base_link, params.models[i].params["frame"]);
      models_[params.models[i].name] = model;
    }
  ...
  for (size_t i = 0; i < data.size(); ++i)
  {
    for (size_t j = 0; j < params.error_blocks.size(); ++j)
    {
      ...
      else if (params.error_blocks[j].type == "chain3d_to_camera2d")
      {
        ...
        std::string model_3d = static_cast<std::string>(params.error_blocks[j].params["model_3d"]);
        std::string model_2d = static_cast<std::string>(params.error_blocks[j].params["model_2d"]);
        double scale = params.getParam(params.error_blocks[j], "scale", 1.0);

        // Do some basic error checking for bad params
        if (model_3d == "" || model_2d == "" || model_3d == model_2d)
        {
          ROS_ERROR("chain3d_to_camera2d improperly configured: model_3d and model_2d params must be set!");
          return 0;
        }

        // Check that this sample has the required features/observations
        if (!hasSensor(data[i], model_3d) || !hasSensor(data[i], model_2d))
          continue;

        // Have to cast our Camera2d model
        auto camera_model = dynamic_cast<Camera2dModel*>(models_[model_2d]);
        if (!camera_model)
        {
          ROS_ERROR("camera2d model is improperly specified");
          return 0;
        }

        // Create the block
        ceres::CostFunction * cost = Chain3dToCamera2d::Create(models_[model_3d],
                                                               camera_model,
                                                               scale,
                                                               offsets_.get(),
                                                               data[i]);
